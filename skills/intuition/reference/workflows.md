...
2. createAtoms([stringToHex("Ethereum")], [atomCost])  value=atomCost
   -> returns [atomId]
...
3. createTriples([subjectId], [predicateId], [objectId], [tripleCost])  value=tripleCost
   -> returns [tripleId]
```
...
4. createTriples([subjectId], [isPredicateId], [aiAgentObjectId], [tripleCost])  value=tripleCost
```
...
```
1. createAtoms([stringToHex("Alice"), stringToHex("trusts"), stringToHex("Bob")], [atomCost, atomCost, atomCost])
   value = atomCost * 3
   -> returns [aliceId, trustsId, bobId]
2. createTriples([aliceId], [trustsId], [bobId], [tripleCost])  value=tripleCost
   -> returns [tripleId]
```
